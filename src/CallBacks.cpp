...
void addWagonCB(Fl_Widget*, TrainWindow* tw)
{
	int lastIndex = tw->trainView->wagons[tw->trainView->wagons.size() - 1].getIndex();
	tw->trainView->wagons.push_back(TrainWagon(lastIndex + 1, 255, 255, 255));
	tw->damageMe();
}

//remove wagons from the train
void deleteWagonCB(Fl_Widget*, TrainWindow* tw)
{
	if (!tw->trainView->wagons.empty())
	{
		tw->trainView->wagons.pop_back();
		tw->damageMe();
	}
}
