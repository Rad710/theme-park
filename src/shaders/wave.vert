...
void main()
{
    ...
    vec3 tangent = vec3(1 - d.x * d.x * (amplitude * sin(f)), d.x * (amplitude * cos(f)),
                        -d.x * d.y * (amplitude * sin(f)));

    vec3 binormal = vec3(-d.x * d.y * (amplitude * sin(f)), d.y * (amplitude * cos(f)),
                         1 - d.y * d.y * (amplitude * sin(f)));
